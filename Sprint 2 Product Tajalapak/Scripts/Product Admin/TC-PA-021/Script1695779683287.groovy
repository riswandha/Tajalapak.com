...
List<WebElement> dataList = WebUI.findWebElements(findTestObject('Object Repository/Product Admin/Data Semua Product'), 10)
...
for(i = 0; i< dataList.size(); i += 7) {
	...
	if(nomor.contains(cariKata)) {
		cekSearch = true
		break;
	}
	if(namaProduct.contains(cariKata)) {
		cekSearch = true
		break;
	}
	if(modal.contains(cariKata)) {
		cekSearch = true
		break;
	}
	if(reseller.contains(cariKata)) {
		cekSearch = true
		break;
	}
	if(konsumen.contains(cariKata)) {
		cekSearch = true
		break;
	}
	if(stok.contains(cariKata)) {
		cekSearch = true
		break;
	}
	if(satuan.contains(cariKata)) {
		cekSearch = true
		break;
	}
	if(berat.contains(cariKata)) {
		cekSearch = true
		break;
	}
	if(status.contains(cariKata)) {
		cekSearch = true
		break;
	}
}
